...
# Prefixes of assignments known to be private (see assignment_config.json).
# Includes both pandas prefixes still in use (fm4-pandas-* and the older
# fm5-pandas-*). fm3-python-programming-* (FM2) was public until students
# were found to be able to browse each other's public repos;
# convert_fm2_to_private.py handles the one-time visibility flip for
# existing repos, this list just needs to include the prefix so future runs
# of this script also cover them. M2A is excluded -- its template is
# empty/unusable, so no valid student repos exist for it yet.

...
(Feature Engineering) for the Foundations course (course_id 21), and `M2B`,
`M2C`, `M3A`, `M3B`, `M3C`, `M3D`, `M4`, `M5A`, `M5B` for the Machine Learning
Engineer course (course_id 19). `M2A` is intentionally unsupported: its
template repo (`mle-m2a-regression-metrics-template`) is empty and not marked
as a usable template — needs its content sourced before it can be added.
Pushing a change to `roster.csv` on `main` triggers the `Provision student
repos` workflow automatically; it skips any repo that already exists, so it's
safe to leave old rows in place.
